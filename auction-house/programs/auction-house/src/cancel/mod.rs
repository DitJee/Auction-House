...
pub fn cancel<'info> (ctx: Context<'_, '_, '_, 'info, Cancel<'info>>, buyer_price: u64, token_size: u64) -> Result<()> {
...
pub fn cancel_logic<'info>(
    ...
    let trade_state_bump = trade_state.try_borrow_data()?[0];
    ...
    let (fee_payer, fee_payer_bump) = get_fee_payer(
        authority, 
        auction_house, 
    ...
    **fee_payer.lamports.borrow_mut() = fee_payer.lamports().checked_add(curr_lamp).ok_or(AuctionHouseError::NumericalOverflow)?;
